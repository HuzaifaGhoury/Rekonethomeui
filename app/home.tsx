...
const Home = () => {
  ...
  return (
    ...
    <div className="relative flex justify-between items-center gap-3 mt-5 w-auto">
        ...
        <div className="flex gap-4 pt-1 ml-4 ">
        ...
        {storyData.map(story => (
          <div key={story.id} className="flex flex-col gap-2">
            <div className="w-full h-40 rounded-md ">
              <Image src={story.storyimage} alt={story.name} width={150} height={150} className="border-4 border-blue-500 rounded-2xl" />
              <div className="flex -m-7 font-inter text-sm font-normal text-slate-50 pl-9 gap-1">
                <div>
                  <Image src={story.image} alt="image" width={25} height={25} className="-mt-2 ml-1 rounded-md" />
                </div>
                <div>
